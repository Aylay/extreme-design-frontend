<script lang="ts">
  ...
  import SvelteMarkdown from 'svelte-markdown'
    ...
    console.log(window.innerWidth)
      ...
      const legalsDiv = document.getElementById(text.id + '-' + text.title.split(' ')[0].toLowerCase());
			if (legalsDiv) {
...
<div class="max-lg:hidden lg:h-[96px]" />
  ...
  <div class="sticky top-[70px] flex flex-nowrap gap-[16px] lg:hidden border-b border-solid border-shark border-opacity-20 w-full max-lg:px-[16px] bg-white pt-[56px]">
    {#each texts as text}
      <h2 class="text-m2 font-medium pb-[14px] {isActiveLegalsMobile ===
        text.id
          ? "text-shark relative before:w-full before:h-[1px] before:bg-shark before:content-[''] before:absolute before:inset-x-0 before:bottom-[-1px]"
          : 'text-silver'}" on:click|preventDefault={() => scrollToDiv(text.id + '-' + text.title.split(' ')[0].toLowerCase())}>
...
<div
	class="flex justify-between max-lg:mt-[56px] pb-[56px] max-lg:flex-col lg:gap-[48px] lg:px-[48px] lg:pb-[96px]"
>
  <div class="flex flex-1 flex-col relative max-lg:hidden">
    <div class="sticky top-[96px]">
      {#each texts as text}
        <h2 class="text-medium font-medium hover:text-shark cursor-pointer lg:w-1/2 {isActiveLegals ===
          text.id
            ? 'text-shark'
            : 'text-silver'}" on:click|preventDefault={() => scrollToDiv(text.id + '-' + text.title.split(' ')[0].toLowerCase())}>
          {@html text.title}
	...
	<div class="flex flex-1 flex-col gap-[56px] lg:gap-[96px] max-lg:px-[16px]">
    {#each texts as text}
    <div id={text.id + '-' + text.title.split(' ')[0].toLowerCase()}>
      <h2 class="font-medium text-[24px] lg:text-[32px] max-lg:leading-[32px] -tracking-[0.03em] mb-[32px]">
        {text.title}
      </h2>
